...
router.get('/', function(req, res) {
  // we first check if the 'contacts' table exists
   pool.query("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'contacts')", function(err,results) {
    if (err) {
      console.log(err);
